...
def download_files(url, path):
    ...
    main = bs4.BeautifulSoup(response.text, "html.parser")

    os.chdir(path) # change the directory
    # Get all pdfs
    itemList = main.findAll('a')
    for i in range(len(itemList)):
        if itemList[i].get('href').endswith('.pdf'):
    ...
    for i in range (len(forDL)):
        ...
        html = stylesheet.prettify("utf-8") + content.prettify("utf-8")
...
if __name__ == "__main__":
	...
	# Retrieve the webpage
	main = bs4.BeautifulSoup(res.text, "html.parser")
